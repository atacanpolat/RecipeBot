...
const ReviewComponent = ({ recipe, token }) => {
  ...
  const fetchReviews = async () => {
    try {
      const updatedReviews = await reviewService.getReviewsByRecipe(
        recipe,
        token
      );
  ...
  useEffect(() => {
    const currentUserReview = reviews.find(
      (review) => review.createdBy === user._id
    );
  ...
  return (
    <ReviewContainer>
      ...
      <h2>Reviews</h2>
      {reviews.length === 0 && <p>It's quite empty in here</p>}
      <ReviewList>
            ...
            <ReviewItem key={review._id}>
              <ReviewHeader>
                <ProfileImage
                  src={
                    review.user.avatar !== ""
                      ? "http://localhost:8000/" + review.user.avatar
                      : URL.createObjectURL(silhouettePP)
                  }
                  alt="Profile"
                />
                <ReviewUsername>
